...
def initiator(sim: rebound.Simulation):
    ...
    sim.add(
        m=7.348e22, a=2.564e-6, primary=sim.particles[0]
    )  # .348e22, a=2.564e-6, primary=sim.particles[1]) # Moon Aa-1 ("Luna"), orbiting planet Aa
    sim.move_to_com()
...
def main(sim: rebound.Simulation):
    ...
    op_earth_moon = rebound.OrbitPlot(
        sim,
        particles=[1],
        primary=0,
        show_primary=True,
