...
T_STAR_SEC: float = 365.25 * 86400 / (2 * np.pi)  # ≈ 5.023e6 s ≈ 58.15 jours
...
class CR3BPSimulation:
    ...
    def __init__(
        ...
        self.times: np.ndarray | None = None  # adim.
        self.states: np.ndarray | None = None  # shape (N, 6)
        self.jacobi: np.ndarray | None = None  # shape (N,)
        self.T_halo: float | None = None  # période adim.
        self.state0: np.ndarray | None = None  # état initial

        # Corps JWST (pour compatibilité avec le module visualization)
        self.jwst = Body(
            "JWST", mass=6500.0, position=np.zeros(3), velocity=np.zeros(3)
        )

    def run(self):
        """Lance l'intégration."""
        state0, T_half = richardson_halo_L2(self.Az, self.mu, self.northern, self.phi)
        ...
        self.times, self.states = integrate(f, state0, 0.0, t_end, h)
    ...
    def L2_position(self) -> np.ndarray:
